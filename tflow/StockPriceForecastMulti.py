...
train_open = close_price[:split_time]

train_data = np.concatenate([train_open,train_volume],axis=1)
#train_data = np.concatenate([train_open],axis=1)
totalVars+=1
window_size = 10
batch_size = 32
...
rnn_forecast = close_price[:, 1][split_time - window_size:split_time]
...
for time in range((len(close_price) - split_time)):
    ...
    input = np.concatenate([close_test, close_price_ema_input, vol_test], axis=1).reshape(-1, window_size, totalVars)
...
"""rnn_forecast = close_price[:, 1][split_time - window_size:split_time]
train_open_conc = train_open[:,0][:-window_size].reshape(-1,1)
